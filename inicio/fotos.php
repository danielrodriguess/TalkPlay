...
?>
		<html>
			<head>
				...
				<title>Minhas fotos - Talkplay</title>
			...
			<body bgcolor='#DCDCDC' link="#E1E1E1" alink="#E1E1E1" vlink="#E1E1E1" onload="tempoatualiza();">
				...
				?>
					<div class='mostrarfotos' style='z-index:1; overflow: auto'>
				<?php
					$_pegarfotos = mysql_query("select * from fotos where email = '$_email'");
					$_contarpegarfotos = mysql_num_rows($_pegarfotos);
					if($_contarpegarfotos > 0){
						for($_i=0;$_i<$_contarpegarfotos;$_i++){
							$_resultado = mysql_fetch_array($_pegarfotos);
							$_imagem= $_resultado['imagem'];
							echo "<form method='post' action='deletarimagem.php'>
							<input type='hidden' value='$_imagem' name='imagem'>
							<input type='hidden' value='$_email' name='email'>
							<input type='hidden' value='$_nick' name='nick'>
							<input type='submit' value='Deletar imagem' id='deletarimagem'></form>";
							if($_caminho == $_imagem){
								
							}else{
								echo "<form method='post' action='alterarimagem.php'>
								<input type='hidden' value='$_imagem' name='imagem'>
								<input type='hidden' value='$_email' name='email'>
								<input type='hidden' value='$_nick' name='nick'>
								<input type='submit' value='Usar com foto de perfil' id='novafoto'>
								</form>";
							}
							echo "<center><img src='../fotodeperfil/$_imagem' width='250px' height='250px'></img><br><br><br>";
							echo "</center>";
						}
					}
