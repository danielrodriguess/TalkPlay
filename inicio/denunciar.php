<?php
	...
	if(!isset($_SESSION['email']) && !isset($_SESSION['senha'])){
		...
		$_pegarblok = mysql_query("select * from denuncia where emailquedenunciou = '$_email' and descricao IS NULL and numero IS NULL");
		$_contarpegarblok = mysql_num_rows($_pegarblok);
		if($_contarpegarblok > 0){
?>
		<html>
			<head>
				<style type="text/css"> 
				...
				<title>Denúcia de pessoas - Talkplay</title>
			...
			<body bgcolor='#DCDCDC' link="#E1E1E1" alink="#E1E1E1" vlink="#E1E1E1" onload="tempoatualiza();">
				...
				?>
				<center><font face='Arial'>Descreva o motivo de sua denúncia</font></center>
				<?php
				echo "<form method='post' action='enviardenuncia.php'>
					<center><textarea rows='9' cols='50' name='reclamacao'></textarea></center>
					<input type='hidden' value='$_email' name='meuemail'>
					<input type='submit' value='Denunciar' id='btnbloquear1'>
				</form>";
				?>
			</body>
		</html>
<?php
		...
		echo "<script type='text/javascript'>alert('Selecione uma pessoa para denunciar')</script>";
